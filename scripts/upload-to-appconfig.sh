...
create_application() {
    ...
    APP_ID=$(aws $aws_profile appconfig list-applications --query "Items[?Name=='$APP_NAME'].Id" --output text 2>/dev/null || echo "")
    
    if [ -z "$APP_ID" ]; then
        log "Creating AppConfig application: $APP_NAME"
        APP_ID=$(aws $aws_profile appconfig create-application \
            --name "$APP_NAME" \
...
# Function to get AWS profile/role for environment
get_aws_profile_for_env() {
    local env_name=$1
    
    case $env_name in
        "dev")
            echo "--profile dev-profile"  # Update with your actual dev profile
            ;;
        "qa")
            echo "--profile qa-profile"   # Update with your actual qa profile
            ;;
        "prod")
            echo "--profile prod-profile" # Update with your actual prod profile
            ;;
        *)
...
process_environment() {
    local env_name=$1
    local aws_profile=$(get_aws_profile_for_env "$env_name")
    
    log "Processing environment: $env_name"
    if [ -n "$aws_profile" ]; then
        log "Using AWS profile/credentials for $env_name"
    fi
    
    create_appconfig_environment "$env_name" "$aws_profile"
    create_configuration_profile "$env_name" "$aws_profile"
    upload_configuration "$env_name" "$aws_profile"
